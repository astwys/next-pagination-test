...
const fetchOrders = async (page = 1, limit = 2, filter: string | undefined) => {
...
export default function Home() {
  ...
  form.useSubmit((state) => {
    ...
    queryParams.page = "1";
    if (filterValue) {
      push({ query: { ...queryParams, filter: filterValue } }, undefined, {
        shallow: true,
      });
    } else {
  ...
  return (
    <div>
      <Form store={form}>
        ...
        <FormInput name={form.names.filter} placeholder="search..." />
        <FormSubmit className="hidden">Submit</FormSubmit>
      </Form>
      <div className="flex gap-4">
        <Button onClick={onChangePage(1)} disabled={meta?.current_page === 1}>
          first
        </Button>
        <Button
